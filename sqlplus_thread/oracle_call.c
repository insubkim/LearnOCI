...
static void    clean_up(int exit_code, t_oci oci) {
    int ret = OCISessionEnd(oci.service_hp, oci.error_hp, 0, OCI_DEFAULT);
    ret = OCIHandleFree(oci.service_hp, OCI_HTYPE_SVCCTX);
    if (!OCI_CHKSUCC(ret)) {
    ...
    ret = OCIHandleFree(oci.env_hp, OCI_HTYPE_ENV);
    if (!OCI_CHKSUCC(ret)) {
        fprintf(stderr, "OCIHandleFree: failed\n");
    }
    printf("clean up\n");
...
static int set_connection(t_oci* ocip) {
    int ret = OCI_ERROR;
    //oci 환경 초기화
    ret = OCIEnvCreate(&(ocip->env_hp), OCI_DEFAULT, 
